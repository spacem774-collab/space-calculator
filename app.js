(function () {
  ...
  const SORTAMENTS = [
    ...
    },
    {
      ...
      name: 'Швеллер',
      fields: [
        { id: 'channelType', label: 'Тип швеллера', unit: '', options: ['У/П', 'Э'] },
        { id: 'N', label: 'Номер швеллера', unit: '' },
        { id: 'L', label: 'Длина', unit: 'м' }
      ],
      hint: 'ГОСТ 8240-97: У/П (уклон/параллельные полки) или Э (экономичный). Укажите длину.',
      calc: function (params, rho) {
        const n = Number(params.N);
        const tbl = params.channelType === 'Э' ? CHANNEL_ECONOMY_MASS : CHANNEL_MASS;
        const kgPerM = tbl[n];
    ...
    },
    {
      ...
      name: 'Двутавр',
      fields: [
        { id: 'ibeamType', label: 'Тип двутавра', unit: '', options: ['8239 (уклон полок)', '26020 Б', '26020 Ш', '26020 К'] },
        { id: 'N', label: 'Номер двутавра', unit: '' },
        { id: 'L', label: 'Длина', unit: 'м' }
      ],
      hint: 'ГОСТ 8239-89 или ГОСТ 26020-83 (Б — нормальный, Ш — широкополочный, К — колонный). Укажите длину.',
      calc: function (params, rho) {
        const n = Number(params.N);
        let tbl = IBEAM_MASS;
        if (params.ibeamType === '26020 Б') tbl = IBEAM_26020_B;
        else if (params.ibeamType === '26020 Ш') tbl = IBEAM_26020_SH;
        else if (params.ibeamType === '26020 К') tbl = IBEAM_26020_K;
        const kgPerM = tbl[n];
        if (kgPerM == null) return null;
        return kgPerM * params.L * (rho / 7850);
      },
  ...
  function channelNOptions() {
    const keys = new Set([].concat(Object.keys(CHANNEL_MASS), Object.keys(CHANNEL_ECONOMY_MASS)));
  ...
  function renderParams() {
    ...
    paramsContainer.innerHTML = sort.fields.map(function (f) {
      ...
      const opts = f.id === 'N' && sort.id === 'channel'
        ? channelNOptions()
        : f.id === 'N' && sort.id === 'ibeam'
          ? ibeamNOptions()
          : '';
      if (opts) {
        return '<div class="param-row"><label for="param-' + f.id + '">' + f.label + '</label>' +
  ...
  function getParams() {
    ...
    sort.fields.forEach(function (f) {
      ...
      if (el.tagName === 'SELECT') {
        ...
        if (f.id === 'channelType' || f.id === 'ibeamType') {
          params[f.id] = raw;
        } else {
          params[f.id] = num;
        }
  ...
  function calculate() {
    ...
    const linearSortaments = ['pipe_round', 'circle', 'angle', 'channel', 'ibeam', 'pipe_rect', 'square', 'rebar', 'strip'];
